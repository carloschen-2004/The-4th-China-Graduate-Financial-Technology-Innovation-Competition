...
from utils.data_clean import auto_clean_all
from utils.feature_align import align_product_features,save_aligned
from utils.product_cluster import prod_cluster
...
def train_for_category(cat, args, cust_df, prod_aligned, event_df):
    ...
    levels = event_df['event_level'].unique()
    start = time.time()
    ...
    if merged.shape[0] == 0:
        ...
        return
    merged, test_merged = train_test_split(
        merged, test_size=0.2, random_state=42
    )
    os.makedirs(os.path.join(work_dir,'merged'),exist_ok=True)
    merged.to_csv(os.path.join(work_dir,f'merged/merged_of_Prod{cat}_train.csv'), index=False)
    test_merged.to_csv(os.path.join(work_dir,f'merged/merged_of_Prod{cat}_test.csv'), index=False)

    log(f"[MERGE] merged_of_Prod{cat}.csv saved in merged")
    # 用户和产品特征
    ...
    user_cols = ([c for c in cust_feats if c in merged.columns and merged[c].std() > 0]
                 + [f'prev_count_{cat_name}' for cat_name in cats]
                 + [f'prev_count_{c}_neg' for c in cats]
    ...
    user_cols = [u for u in user_cols if u in merged.columns and merged[u].std() > 0]
