...
def train_for_category(cat, args, cust_df, prod_aligned, events_df):
    ...
    # select successful events A/B
    ev = events_df[events_df['A']==1].copy()
    ...
    if merged.shape[0] == 0:
        log(f"[TRAIN] Merged empty for {cat} after join; possible id mismatch"); return
    # user features: all cust cols except cust_no
    cust_feats = [c for c in cust_df.columns if c != 'cust_no']
    user_cols = [c for c in cust_feats if c in merged.columns]
    user_cols = [c for c in user_cols if merged[c].std() > 0]
    if len(user_cols) == 0:
        log(f"[TRAIN] No usable user features for {cat}"); return
    prod_cols_in_merged = [c for c in prod_cols if c in merged.columns]
    prod_cols_in_merged = [c for c in prod_cols_in_merged if merged[c].std() > 0]
    if len(prod_cols_in_merged) == 0:
        log(f"[TRAIN] No usable product features for {cat}"); return
    ...
    prod_dim = len(prod_cols_in_merged) if len(prod_cols_in_merged)>0 else 1
    log(f"[INFO] {cat} user_dim={user_dim} prod_dim={prod_dim} samples={len(ds)}")
    ...
    for epoch in range(args.epochs):
        ...
        for users, prods in tqdm(loader, desc=f"{cat} epoch {epoch+1}"):
            ...
            optimizer.zero_grad()
            u = user_t(users); p = prod_t(prods)
            u = torch.nn.functional.normalize(u, dim=1)
            p = torch.nn.functional.normalize(p, dim=1)
            logits = torch.matmul(u, p.T) / (args.temperature + 1e-8)
            labels = torch.arange(u.size(0), device=users.device)
            loss = torch.nn.functional.cross_entropy(logits, labels)
            if torch.isnan(loss):
                log(f"[WARN] NaN loss on {cat} epoch {epoch+1}"); loss = torch.tensor(0.0, requires_grad=True)
            loss.backward()
