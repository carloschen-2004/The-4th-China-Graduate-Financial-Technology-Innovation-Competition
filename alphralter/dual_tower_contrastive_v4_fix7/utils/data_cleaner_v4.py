...
def clean_event_df(path):
    ...
    for c in df.columns:
        if c in ['prod_id','cust_no','event_type','A','B','D','is_success','event_dt']: continue
        df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0)
    for c in df.columns:
        if 'date' in str(c).lower() or 'dt' in str(c).lower():
            try:
                df['event_dt'] = pd.to_datetime(df[c], errors='coerce')
                break
            except:
                continue
    return df
